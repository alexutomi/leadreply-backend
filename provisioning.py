...
def buy_twilio_number(business_phone):
    """Buy a Twilio number in the same area code as the business."""
...
async def create_business_account(data: dict):
    ...
    try:
        ...
        # Step 4 — Save business to database
...
# ── PROVISION NUMBER FOR EXISTING AGENCY CLIENT ───────────
async def provision_new_business(business_name, business_phone, auto_reply_message=None):
    """Used by agency dashboard to add a new client."""
    client = Client(TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN)
    try:
        area_code = business_phone.replace("+1", "")[:3]
        available = client.available_phone_numbers("US").local.list(
        ...
        return {
            ...
            "message":       f"Provisioned {new_number.phone_number} for {business_name}"
        }
